...
class QuestionTheme extends LSActiveRecord
{
    ...
    public static function convertLS3toLS5($sXMLDirectoryPath)
    {
        ...
        if (!is_file($sPathToCoreConfigFile)) {
            return $aSuccess = [
                'message' => sprintf(
                    gT("Question theme could not be converted to the latest FormFlex version. Reason: No matching core theme with the name %s could be found"),
                    $sThemeDirectoryName
                ),
                'success' => false
            ];
        ...
        return $aSuccess = [
            'message' => gT('Question theme has been successfully converted to the latest FormFlex version.'),
            'success' => true
        ];
    }
